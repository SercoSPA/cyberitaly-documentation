...
# Ora legale italiana: si usa Europe/Rome via zoneinfo, che sa da solo quando
# scattano ora legale/solare. Se il DB dei fusi non e' disponibile sul server,
# si ripiega su un offset fisso (meno preciso al confine dei cambi d'ora).
try:
...
# Finestra NWD/NWH: lun-ven (weekday 1-5), 08:00-16:59 ora italiana.
# La fascia e' [start, end): 08 incluso, 17 escluso.
NWH_START_HOUR = 8
NWH_END_HOUR = 17
NWD_DAYS = (1, 2, 3, 4, 5)  # ISO: lunedi=1 ... domenica=7


def local_rome(dt: datetime):
    """
    Converte un istante UTC in ora italiana. Ritorna
    (datetime_locale, iso_weekday 1-7, ora_locale 0-23).
    """
    dt = dt.astimezone(timezone.utc)
    if _ROME_TZ is not None:
        loc = dt.astimezone(_ROME_TZ)
    else:
        # fallback grezzo: CET/CEST approssimato con offset fisso +1
        # (non gestisce il cambio d'ora: usato solo se manca il tzdb)
        loc = dt.astimezone(timezone(timedelta(hours=1)))
    return loc, loc.isoweekday(), loc.hour


def is_business_window(dt: datetime) -> bool:
    """True se l'istante cade in NWD (lun-ven) e NWH (08:00-16:59) ora italiana."""
...
class InsulaKpiCollector:
    ...
    def _envelope(self, ts: datetime, event_type: str, started: datetime, ended: datetime):
        ...
        loc, weekday, hour_local = local_rome(ts)
        return {
            "@timestamp": iso_ms(ts),
            ...
            # --- contesto temporale per report/filtri NWD-NWH (ora italiana) ---
            # La raccolta resta h24: questi campi ETICHETTANO soltanto, non filtrano.
            "weekday": weekday,                 # ISO 1=lun ... 7=dom
            "hour_local": hour_local,           # 0-23, ora italiana (DST gestita)
            "hour_utc": ts.astimezone(timezone.utc).hour,
            "is_nwd": weekday in NWD_DAYS,      # giorno lavorativo (lun-ven)
            "is_nwh": is_business_window(ts),   # lun-ven E 08:00-16:59 italiane
        }
    ...
    def build_events(self, started: datetime, ended: datetime, elapsed_ms: float):
        ...
        agg.update({
            ...
            "catalogue_up": 1 if catalogue_status == STATUS_OK else 0,
            "access_status": access_status,
            "access_available": access_status == STATUS_OK,
            "access_up": 1 if access_status == STATUS_OK else 0,
