...
CyberItaly data processing management and development allows you to seamlessly integrate your own algorithm. Generally speaking, it is possible to integrate any program running on Linux.
...
* Visit https://iride-cyberitaly.space/intellect/.
...
Through the input definition tab, the user specifies how the service will expose its input and interact with CyberItaly data processing management and development to get them.
...
CyberItaly data processing management and development will behave as follows with respect to service inputs:
...
The user defines an input with ``rounding`` of type number. A user invokes this service passing as arguments the value ``0.5``. CyberItaly data processing management and development will create inside the container the file ``/home/worker/workDir/FSTEP-WPS-INPUT.properties`` with the following content:
...
2. For any Catalog product, CyberItaly data processing management and development will place the resource content in a subfolder of ``/home/worker/workDir/inDir`` having the input ID as name.
...
The output definition tab defines how the service will interact with the CyberItaly data processing management and development for the service outputs.
...
so that the CyberItaly data processing management and development will be able to extract from the file name and parse the corresponding timestamp (in this case, a date without a time) to represent the product start and end time.
...
so that CyberItaly data processing management and development will be able to extract from the file name and parse the two corresponding timestamps (in this case, dates with a time) to represent the product start and end time.


Behaviour for outputs
^^^^^^^^^^^^^^^^^^^^^

CyberItaly data processing management and development will behave as follows with respect to service outputs:
